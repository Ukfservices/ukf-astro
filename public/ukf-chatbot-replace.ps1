# ============================================================
# UKF — Replace old chatbot with new widget on all pages
# Run from your site root: C:\ukf-services-site\
# ============================================================

$files = @("about.html","contact.html","services.html","testimonials.html","process.html")
...
$widget = @'
    ...
    <style>
      ...
      #ukf-bubble:hover{transform:scale(1.08);}
    ...
    <button id="ukf-bubble" onclick="ukfToggle()" aria-label="Open UKF Smart Assistant">
      <svg viewBox="0 0 26 26" fill="none"><path d="M13 2C7.477 2 3 6.15 3 11.25c0 2.7 1.23 5.13 3.2 6.84L5.5 22l4.56-2.1A11.1 11.1 0 0013 20.5c5.523 0 10-4.15 10-9.25S18.523 2 13 2z" fill="#c9a84c"/><circle cx="9" cy="11" r="1.3" fill="#0a1628"/><circle cx="13" cy="11" r="1.3" fill="#0a1628"/><circle cx="17" cy="11" r="1.3" fill="#0a1628"/></svg>
    </button>
    <div id="ukf-window" role="dialog" aria-label="UKF Smart Assistant">
      <div class="ukfw-header">
        <div class="ukfw-header-left">
          <div class="ukfw-av-wrap"><div class="ukfw-av" id="ukfw-initials">LA</div><div class="ukfw-dot"></div></div>
          <div><div class="ukfw-name" id="ukfw-name">Layla</div><div class="ukfw-role">UKF Freight Intelligence</div></div>
        </div>
        <div class="ukfw-hbtns">
          <button class="ukfw-hbtn" onclick="ukfClear()" aria-label="Clear chat"><i class="fas fa-rotate-right" style="font-size:12px;"></i></button>
          <button class="ukfw-hbtn" onclick="ukfToggle()" aria-label="Close"><i class="fas fa-xmark"></i></button>
        </div>
      </div>
      <div class="ukfw-msgs" id="ukfw-msgs"></div>
      <div class="ukfw-quick">
        <button class="ukfw-qbtn" onclick="ukfAct('volumetric')">Volumetric Weight</button>
        <button class="ukfw-qbtn" onclick="ukfAct('incoterms')">Incoterms</button>
        <button class="ukfw-qbtn" onclick="ukfAct('customs')">UAE Customs</button>
        <button class="ukfw-qbtn" onclick="ukfAct('corridors')">Trade Corridors</button>
        <button class="ukfw-qbtn" onclick="ukfAct('quote')">Get a Quote</button>
      </div>
      <div class="ukfw-input-row">
        <input class="ukfw-input" id="ukfw-input" type="text" placeholder="Ask about freight, customs, routing..." onkeydown="if(event.key==='Enter')ukfSend()" aria-label="Type a message">
        <button class="ukfw-send" onclick="ukfSend()" aria-label="Send"><i class="fas fa-paper-plane" style="font-size:13px;"></i></button>
      </div>
    </div>
    <script>
    (function(){
      ...
      var LEAD_Q={name:"First, what's your name?",route:'Thanks {name}! Origin and destination? (e.g. Dubai to Valencia)',cargo:'What are you shipping? (type + approx. weight or volume)'};
      document.addEventListener('DOMContentLoaded',function(){
        document.getElementById('ukfw-name').textContent=agent.name;
        document.getElementById('ukfw-initials').textContent=agent.initials;
        aiMsg('Hi there! I am <strong>'+agent.name+'</strong> from the UKF team. I can help with freight calculations, Incoterms, UAE customs, and trade routing.<br><br>What can I help you with today?');
      });
      window.ukfToggle=function(){var w=document.getElementById('ukf-window'),b=document.getElementById('ukf-bubble'),open=w.style.display==='flex';w.style.display=open?'none':'flex';b.style.display=open?'flex':'none';if(!open)scroll();};
      window.ukfClear=function(){document.getElementById('ukfw-msgs').innerHTML='';leadStep=0;leadData={};aiMsg('Chat cleared. How can I help with your shipment today?');};
      window.ukfAct=function(type){
        document.querySelectorAll('.ukfw-qbtn').forEach(function(b){b.classList.remove('on');});
        var labels={volumetric:'Volumetric Weight',incoterms:'Incoterms',customs:'UAE Customs',corridors:'Trade Corridors',quote:'Get a Quote'};
        document.querySelectorAll('.ukfw-qbtn').forEach(function(b){if(b.textContent.trim()===labels[type])b.classList.add('on');});
        typing();
        setTimeout(function(){rmTyping();var h='';
          if(type==='volumetric'){h='<div class="ukfw-card"><div class="ukfw-card-hd"><i class="fas fa-box" style="color:var(--ukf-gold);font-size:11px;"></i><span>Air Freight Calculator</span></div><div class="ukfw-card-bd"><div class="ukfw-dim-row"><input type="number" id="ukfw-cl" class="ukfw-dim-in" placeholder="L cm" min="0"><span class="ukfw-dim-sep">x</span><input type="number" id="ukfw-cw" class="ukfw-dim-in" placeholder="W cm" min="0"><span class="ukfw-dim-sep">x</span><input type="number" id="ukfw-ch" class="ukfw-dim-in" placeholder="H cm" min="0"></div><div style="font-size:11px;color:var(--ukf-muted);margin-bottom:9px;">Divisor 6000 - Standard air freight</div><button class="ukfw-calc-btn" onclick="ukfCalc()">Calculate</button><div class="ukfw-result" id="ukfw-res"><span class="ukfw-result-label">Volumetric Weight</span><span class="ukfw-result-val" id="ukfw-res-val">0</span><div class="ukfw-cta-row"><button class="ukfw-cta gold" onclick="ukfAct(\'quote\')">Request Quote</button></div></div></div></div>';}
          else if(type==='incoterms'){h='<strong>Common Incoterms from Jebel Ali:</strong><div style="margin-top:9px;"><div class="ukfw-inco"><div class="ukfw-inco-code">FOB <span style="font-size:11px;font-weight:400;color:var(--ukf-muted);">Free On Board</span></div><div class="ukfw-inco-desc">Seller loads at origin port. Buyer handles freight, insurance and destination costs.</div></div><div class="ukfw-inco"><div class="ukfw-inco-code">DDP <span style="font-size:11px;font-weight:400;color:var(--ukf-muted);">Delivered Duty Paid</span></div><div class="ukfw-inco-desc">UKF handles everything - freight, customs, duties, final delivery.</div></div><div class="ukfw-inco"><div class="ukfw-inco-code">CIF <span style="font-size:11px;font-weight:400;color:var(--ukf-muted);">Cost, Insurance and Freight</span></div><div class="ukfw-inco-desc">Seller covers freight to destination port. Risk transfers at port of loading.</div></div></div><div class="ukfw-cta-row" style="margin-top:8px;"><button class="ukfw-cta gold" onclick="ukfAct(\'quote\')">Help me choose the right term</button></div>';}
          else if(type==='customs'){h='<strong>UAE Customs - Standard Requirements</strong><ul class="ukfw-list"><li><div class="ukfw-num">1</div><div>Commercial Invoice - HS codes, declared value, seller details</div></li><li><div class="ukfw-num">2</div><div>Packing List - weights, dimensions, quantities</div></li><li><div class="ukfw-num">3</div><div>Certificate of Origin - authenticated by Chamber of Commerce</div></li><li><div class="ukfw-num">4</div><div>Bill of Lading or Airway Bill</div></li><li><div class="ukfw-num">5</div><div>Import Permit - required for regulated goods</div></li></ul><div class="ukfw-cta-row" style="margin-top:10px;"><button class="ukfw-cta gold" onclick="ukfAct(\'quote\')">Get customs support</button></div>';}
          else if(type==='corridors'){h='<strong>UKF Active Trade Corridors</strong><div style="margin-top:9px;"><div class="ukfw-corridor"><div class="ukfw-corridor-name">UAE GCC Hub</div><div class="ukfw-corridor-detail">Dubai - Jebel Ali - Abu Dhabi - Saudi Arabia - Kuwait - Oman</div></div><div class="ukfw-corridor"><div class="ukfw-corridor-name">Spain Europe</div><div class="ukfw-corridor-detail">Valencia - UAE to Spain - Spain to GCC - North Africa</div></div><div class="ukfw-corridor"><div class="ukfw-corridor-name">Indonesia ASEAN</div><div class="ukfw-corridor-detail">Jakarta - Indonesia to UAE - ASEAN to GCC - Saudi Arabia</div></div></div><div class="ukfw-cta-row" style="margin-top:8px;"><button class="ukfw-cta gold" onclick="ukfAct(\'quote\')">Get a corridor quote</button></div>';}
          else if(type==='quote'){leadStep=0;leadData={};h='Happy to arrange a quote - just need a few quick details.<br><br>'+LEAD_Q.name;leadStep=1;}
          if(h)aiMsg(h);
        },500);
      };
      window.ukfCalc=function(){var l=parseFloat(document.getElementById('ukfw-cl').value),w=parseFloat(document.getElementById('ukfw-cw').value),h=parseFloat(document.getElementById('ukfw-ch').value),res=document.getElementById('ukfw-res'),val=document.getElementById('ukfw-res-val');if(!l||!w||!h||l<=0||w<=0||h<=0){val.textContent='Enter all dimensions';val.style.fontSize='13px';res.style.display='block';return;}val.style.fontSize='22px';val.textContent=((l*w*h)/6000).toFixed(2)+' kg';res.style.display='block';scroll();};
      window.ukfSend=function(){var inp=document.getElementById('ukfw-input'),txt=inp.value.trim();if(!txt)return;inp.value='';userMsg(txt);typing();setTimeout(function(){rmTyping();if(leadStep>0&&leadStep<=LEAD_STEPS.length){var field=LEAD_STEPS[leadStep-1];leadData[field]=txt;if(leadStep<LEAD_STEPS.length){leadStep++;aiMsg(LEAD_Q[LEAD_STEPS[leadStep-1]].replace('{name}',leadData.name||''));}else{leadStep=0;aiMsg('Thank you, <strong>'+esc(leadData.name)+'</strong>!<br><br><strong>Route:</strong> '+esc(leadData.route)+'<br><strong>Cargo:</strong> '+esc(leadData.cargo)+'<br><br>Our team will be in touch with a tailored quote:'+contactCard());if(typeof gtag!=='undefined'){gtag('event','ukf_lead_captured',{'event_category':'Lead Generation','event_label':'assistant_lead'});}}return;}var lo=txt.toLowerCase();if(/(weight|volume|cbm|kg|dimension)/.test(lo)){ukfAct('volumetric');return;}if(/(incoterm|fob|ddp|cif)/.test(lo)){ukfAct('incoterms');return;}if(/(custom|clearance|document|invoice)/.test(lo)){ukfAct('customs');return;}if(/(corridor|spain|indonesia|valencia|jakarta)/.test(lo)){ukfAct('corridors');return;}if(/(quote|price|cost|rate|how much)/.test(lo)){ukfAct('quote');return;}if(/(human|team|expert|call|email|whatsapp|contact|phone)/.test(lo)){aiMsg('Of course! Here is how to reach us:'+contactCard());return;}aiMsg('I specialise in freight calculations, Incoterms, UAE customs, and trade routing. Try a quick button above!');},500);};
      function contactCard(){return '<div class="ukfw-contact"><div class="ukfw-contact-row"><i class="fas fa-phone ukfw-contact-icon"></i><span><a href="tel:+97155257837">'+CONTACT.phone+'</a></span></div><div class="ukfw-contact-row"><i class="fab fa-whatsapp ukfw-contact-icon"></i><span><a href="'+CONTACT.whatsapp+'" target="_blank">WhatsApp us</a></span></div><div class="ukfw-contact-row"><i class="fas fa-envelope ukfw-contact-icon"></i><span><a href="mailto:'+CONTACT.email+'">'+CONTACT.email+'</a></span></div><div class="ukfw-contact-row"><i class="fas fa-clock ukfw-contact-icon"></i><span>'+CONTACT.hours+'</span></div></div>';}
      function aiMsg(html){var msgs=document.getElementById('ukfw-msgs'),row=document.createElement('div');row.className='ukfw-row ukfw-in';row.innerHTML='<div class="ukfw-row-av">'+agent.initials+'</div><div class="ukfw-col"><div class="ukfw-msg">'+html+'</div><span class="ukfw-time">'+now()+'</span></div>';msgs.appendChild(row);scroll();}
      function userMsg(txt){var msgs=document.getElementById('ukfw-msgs'),row=document.createElement('div');row.className='ukfw-row u ukfw-in';row.innerHTML='<div class="ukfw-col"><div class="ukfw-msg">'+esc(txt)+'</div><span class="ukfw-time">'+now()+'</span></div><div class="ukfw-row-av" style="background:var(--ukf-navy);color:var(--ukf-gold);">You</div>';msgs.appendChild(row);scroll();}
      function typing(){var msgs=document.getElementById('ukfw-msgs'),d=document.createElement('div');d.id='ukfw-typing';d.className='ukfw-row';d.innerHTML='<div class="ukfw-row-av">'+agent.initials+'</div><div class="ukfw-typing"><span></span><span></span><span></span></div>';msgs.appendChild(d);scroll();}
      function rmTyping(){var t=document.getElementById('ukfw-typing');if(t)t.remove();}
      function scroll(){var m=document.getElementById('ukfw-msgs');if(m)m.scrollTop=m.scrollHeight;}
      function now(){return new Date().toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});}
      function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;');}
    })();
    </script>
    <!-- END UKF SMART ASSISTANT -->
</body>
</html>
'@
...
foreach ($file in $files) {
    ...
    # Remove old chatbot block using regex
    $content = $content -replace '(?s)\s*<!--\s*Chatbot Widget\s*-->.*?</div>\s*\n\s*\n', "`n"
    $content = $content -replace '(?s)\s*<div id="chatBubble".*?<div class="chat-footer">.*?</div>\s*\n\s*</div>', ''

    # Replace closing </body></html> with widget
    $content = $content -replace '</body>\s*</html>', $widget
...
Write-Host "git add ."
Write-Host "git commit -m 'Replace old chatbot with UKF Smart Assistant on all pages'"
